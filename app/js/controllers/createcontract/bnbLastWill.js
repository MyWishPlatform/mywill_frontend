angular.module('app').controller('bnbLastWillCreateController', function($scope, contractService, $timeout, $state, $rootScope,
    ...
    $scope.networkName = 'BNB';

    var generateContractData = function() {
        return {
            name: $scope.contractName,
            id: contract.id,
            contract_type: CONTRACT_TYPES_CONSTANTS.BNB_LAST_WILL,
    ...
    var checkDraftContract = function(redirect) {
        if (localStorage.draftContract && !contract.id) {
            if (!contract.id) {
                ...
                if (draftContract.contract_type == CONTRACT_TYPES_CONSTANTS.BNB_LAST_WILL) {
