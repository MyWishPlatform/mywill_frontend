angular.module('app').controller('eosCrowdSaleCreateController', function(
    ...
    var generateContractData = function() {
        ...
        contractDetails.hard_cap = new BigNumber(contractDetails.hard_cap).times(Math.pow(10, contractDetails.decimals)).toFixed().toString(10);

        if (contractDetails.soft_cap) {
            contractDetails.soft_cap = new BigNumber(contractDetails.soft_cap).times(Math.pow(10, contractDetails.decimals)).toFixed().toString(10);
        }
    ...
    $scope.resetForms = function() {
        ...
        if ($scope.request.hard_cap) {
            $scope.request.hard_cap = new BigNumber($scope.request.hard_cap).div(Math.pow(10, $scope.request.decimals)).toFixed().toString(10);
        }
        if ($scope.request.soft_cap) {
            $scope.request.soft_cap = new BigNumber($scope.request.soft_cap).div(Math.pow(10, $scope.request.decimals)).toFixed().toString(10);
        }
        setStartTimestamp();
        setStopTimestamp();
