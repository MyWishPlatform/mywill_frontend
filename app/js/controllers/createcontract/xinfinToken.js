angular.module('app').controller('xinfinTokenCreateController', function($scope, contractService, $timeout, $state, $rootScope, NETWORKS_TYPES_CONSTANTS,
                                                                        CONTRACT_TYPES_CONSTANTS, openedContract, $stateParams, currentUser) {

    var user = currentUser.data;

    if (openedContract.data !== undefined){
        if (openedContract.data.contract_details.admin_address.slice(0,2) === "0x"){
            openedContract.data.contract_details.admin_address = "xdc" + openedContract.data.contract_details.admin_address.slice(2);
        }
    }
    ...
    var generateContractData = function() {
        ...
        return {
            ...
            contract_type: CONTRACT_TYPES_CONSTANTS.XINFIN_TOKEN,
    ...
    var checkDraftContract = function(redirect) {
        if (localStorage.draftContract) {
            if (!contract.id) {
                ...
                if (draftContract.contract_type == CONTRACT_TYPES_CONSTANTS.XINFIN_TOKEN) {
